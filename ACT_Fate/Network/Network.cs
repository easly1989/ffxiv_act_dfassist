        ...
        */
        internal void StartCapture(Process process)
        {
            ...
            Task.Factory.StartNew(() =>
            {
                try
                {
                    Log.I("[" + pid + "]l-network-starting");

                    if (IsRunning)
                    {
                        Log.E("[" + pid + "]l-network-error-already-started");
                        return;
                    }
                    ...
                    {
                        Log.E("[" + pid + "]l-network-error-no-connection");
                        return;
                    }
                    ...
                    Log.S("[" + pid + "]l-network-started:" + localAddress);
                }
                catch (Exception ex)
                {
                    Log.Ex(ex, "[" + pid + "]l-network-error-starting");
                }
            });
        }
        ...
        internal void StopCapture()
        {
            try {
                if (!IsRunning)
                {
                    Log.E("[" + pid + "]l-network-error-already-stopped");
                    return;
                }
                ...
                Log.I("[" + pid + "]l-network-stopping");
            }
            catch (Exception ex)
            {
                Log.Ex(ex, "[" + pid + "]l-network-error-stopping");
            }
        }
        ...
        internal void UpdateGameConnections(Process process)
        {
            ...
            foreach (var connection in connections)
            {
                if (!currentConnections.Contains(connection))
                {
                    ...
                    update = true;
                    Log.E("[" + pid + "]l-network-detected-connection-closing");
                    break;
                }
            }
            ...
            if (update)
            {
                ...
                foreach (var connection in connections)
                {
                    Log.I("[" + pid + "]l-network-detected-connection: " + connection.ToString());
                }
            }
        }
        ...
        private void OnReceive(IAsyncResult ar)
        {
            ...
            catch (Exception ex) when (ex is ObjectDisposedException || ex is NullReferenceException)
            {
                IsRunning = false;
                socket = null;
                Log.S("[" + pid + "]l-network-stopped");
            }
            catch (Exception ex)
            {
                Log.Ex(ex, "[" + pid + "]l-network-error-receiving-packet");
            }
        }
        ...
        private void FilterAndProcessPacket(byte[] buffer)
        {
            try {
                ...
                if (ipPacket.IsValid && ipPacket.Protocol == ProtocolType.Tcp)
                {
                    ...
                    */
                    //Log.I("[" + pid + "]l-network-filtering-packet " + sourceEndPoint + ", " + destinationEndPoint);
            ...
            {
                Log.Ex(ex, "[" + pid + "]l-network-error-filtering-packet");
            }
        }
        ...
        private IPEndPoint GetLobbyEndPoint(Process process)
        {
            ...
            {
                Log.Ex(ex, "[" + pid + "]l-network-error-finding-lobby");
            }
